...
class ACCESO 
  ...
  public function cargaMatriculaEspecial($fecha) 
	...
	if (($gestor = fopen($this->csv_file, "r")) !== FALSE) 
	{
      ...
      //preparando datos
		$m_matriculado=intval($matricula[9]);//si está o no matriuclado o casua baja	
      if($m_matriculado==0)
         continue;
		if($matricula[0]=='CE') continue;
		$total_filas++;		
      $matricula[0]=str_replace('"','',$matricula[0]);	
      $m_curso=str_replace('"','',$matricula[0]);	
		$m_curso=utf8_encode($m_curso);	

		$m_provincia=$matricula[1];	
		$m_ensenanza=mysqli_real_escape_string($this->c,$matricula[5]);	

		$m_nombre=$matricula[6];	
		$m_apellidos=$matricula[7];	
		$m_fnac=$this->make_fecha($matricula[8]);	
		$m_tipo_centro=$matricula[2];	
		$m_nombre_centro=$matricula[3];
      ...
      $m_codigo_centro=str_replace('"','',$m_codigo_centro);	
		...
		if($edad>=$edadtva) 
		{
         print("FECHA: \n");
         print($m_fnac);
			print("ALUMNO ERRONEO, EDAD MAYOR Q TVA, edad: ".$edad."\n");
			print_r($matricula);
			$nedad++;
			continue;
		}
		else 
		{
         print("FECHA: \n");
         print($m_fnac);
			print("ALUMNO CORRECTO, EDAD MAYOR Q TVA, edad: ".$edad."\n");
			print_r($matricula);
			$nedad++;
		}
  ...
  public function make_fecha($s) 
	{
      //formato dia-mes-año
      $afecha=explode("/",$s);
      $sfecha=$afecha[0]."-".$afecha[1]."-".$afecha[2];
