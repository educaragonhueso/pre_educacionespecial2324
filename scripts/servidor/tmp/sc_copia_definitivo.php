...
$log_fase_definitivo=new logWriter('log_fase_definitivo',DIR_LOGS);
...
//$ct=$tsolicitud->copiaTablaCentro(0,'alumnos_definitivo');	
...
foreach($acentros as $dcentro)
{
	...
	########################################################################################
	$log_fase_definitivo->warning("INICIANDO GESTION CENTRO");
	$log_fase_definitivo->warning(print_r($dcentro,true));
	########################################################################################
   ...
   ########################################################################################
	$log_fase_definitivo->warning("NOMBRE: ".$nombrecentro.PHP_EOL);
	$log_fase_definitivo->warning("FASE: ".$centrotmp->getFaseSorteo().PHP_EOL);
	$log_fase_definitivo->warning("NSOLICITUDES: ".$nsolicitudescentro.PHP_EOL);
   ########################################################################################

	$dsorteo=$centrotmp->getVacantes('centro',$log_fase_definitivo);
	$vacantes_ebo=$dsorteo[0]->vacantes;
	$vacantes_tva=$dsorteo[1]->vacantes;
	$log_fase_definitivo->warning("VACANTES EBO: $vacantes_ebo".PHP_EOL);
...
$ct=$tsolicitud->copiaTablaCentro(1,'alumnos_definitiva',$log_fase_definitivo);	
$log_fase_definitivo->warning("RESULTADO COPIAR TABLA $ct ");
/*
########################################################################################
########################################################################################
//Si hemos llegado al dia d elas definitivoes o posterior, generamos la tabla de soliciutdes para los listados definitivoes
$acentros=array();
$centros=$ccentros->getAllCentros('todas','especial');
		while($row = $centros->fetch_assoc()) { $acentros[]=$row;}
		foreach($acentros as $dcentro)
...
//PARA LOS CENTROS Q NO HAYAN HECHO SORTEO, OSE A Q ESTEN EN FASE <2, LO HACEMOS NOSOTROS
		$log_definitivo_fase2->warning("ENTRANDO SORTEO TABLA CENTRO: $nombrecentro");
		...
		if($fase<2) //centros q no hayan realizado el sorteo
		{
			...
			########################################################################################
			$log_definitivo_fase2->warning("SORTEO REALIZADO");
			########################################################################################
			$modo='sorteo';
		...
		$ct=$tsolicitud->copiaTablaCentro($id_centro,'alumnos_definitivo_final');	
		}
*/
