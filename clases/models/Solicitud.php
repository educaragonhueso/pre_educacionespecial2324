...
desc,a.tutores_centro desc,a.nordensorteo asc,a.nasignado desc";
	...
	public function getSolicitudesFase2Finales($subtipo_listado,$rol,$id_centro,$estado_convocatoria,$log,$id_alumno) 
   {
      ...
      if($rol=='centro')
         $where=" AND id_centro_final=$id_centro AND est_desp_sorteo IN('admitida','admitidafase2') ";
      if($rol=='admin')
         $where=" AND est_desp_sorteo IN('admitida','admitidafase2') ";
      
      $resultSet=array();
		if($subtipo_listado=='lfinal_sol_ebo')
		   $sql="SELECT 'centrosdisponibles' as centrosdisponibles,a.id_alumno,a.tipoestudios,a.id_centro_final, a.apellido1,a.apellido2,a.nombre,c.nombre_centro as centro_definitivo FROM alumnos a,centros c WHERE c.id_centro=a.id_centro_final AND a.tipoestudios='ebo' $where order by a.id_centro_final desc, a.tipoestudios asc,a.nordensorteo asc,a.transporte asc";
   	elseif($subtipo_listado=='lfinal_sol_tva')
		   $sql="SELECT 'centrosdisponibles' as centrosdisponibles, a.* FROM alumnos a where a.tipoestudios='tva' $where order by a.id_centro_final desc, a.tipoestudios asc,a.nordensorteo asc,a.transporte asc";
