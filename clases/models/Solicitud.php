...
class Solicitud {
  ...
  public function copiaTablaBaremacion($centro,$tipoa='alumnos_baremacion_final',$tipob='baremo_baremacion_final')
   ...
   public function copiaTablaFase2Final()
   {
   ...
   public function copiaTablaFase2()
   {
      //generamos la tabla de fase2 par alas asignaciones de plazas restantes
	   $sql="SELECT a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.est_desp_sorteo,c.nombre_centro,b.*,a.id_centro_destino as id_centro_final FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro where fase_solicitud!='borrador'";
	   //volcamos la tabla con los datos de solicitudes y los del baremo tal como aparecen en el listado de provisionales o definitivosa
			$dsql='DELETE from alumnos_fase2';
			$isql="INSERT IGNORE INTO alumnos_fase2 $sql";
		if($this->conexion->query($dsql))
			if($this->conexion->query($isql)) return 1;
			else return $this->conexion->error;
		else return $this->conexion->error;
	}
   public function copiaTablaFase2Final_old($log)
   {
		$sql="SELECT a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.est_desp_sorteo,c.nombre_centro,b.*,a.id_centro_destino as id_centro_destino FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro where fase_solicitud!='borrador'";
	//volcamos la tabla con los datos de solicitudes y los del baremo tal como aparecen en el listado de provisionales o definitivosa
		if($centro!=1)
  ...
  public function copiaTablaCentro($centro,$tabla_destino,$log)
	{
		$sql="SELECT a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.est_desp_sorteo,c.nombre_centro,b.*,a.id_centro_destino as id_centro_destino FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro WHERE fase_solicitud!='borrador'";
	//volcamos la tabla con los datos de solicitudes y los del baremo tal como aparecen en el listado de provisionales o definitivosa
	   $csql='CREATE TABLE IF NOT EXISTS '.$tabla_destino.' '.$sql;
		if(!$this->conexion->query($csql)) return 0;
		if($centro!=1)
		{
			$dsql='DELETE from '.$tabla_destino.' WHERE id_centro_destino='.$centro;
			$isql='INSERT IGNORE INTO '.$tabla_destino.' '.$sql.' and id_centro_destino='.$centro;
...
We can now print a cell with Cell(). A cell is a rectangular area, possibly framed, which contains a line of text. It is output at the current position. We specify its dimensions, its text (centered or aligned), if borders should be drawn, and where the current position moves after it (to the right, below or to the beginning of the next line). To add a frame, we would do this: ";
  ...
  public function update($sol,$id,$token,$rol,$estado_convocatoria,$log)
  {
      ...
      */
      ...
      //modificamos datos del alumno solo si estamos en periodo de inscripcion
      if($estado_convocatoria==ESTADO_INSCRIPCION)
      {
         //primero comprobamos si es conjunta, si no ya no lo es desactivamos la opción en todos los hermanos y marcamos como borrador la d elos hermnoas
			   if($sol['conjunta']=='no')
            {
               $log->warning("SOLICITUD NO CONJUNTA");
               $dni_tutor1=$sol['dni_tutor1'];
               $idhs=$this->getIdsHermanosAdmision($id,$log);

               foreach($idhs as $idh)
               {
                  $sql="UPDATE alumnos SET conjunta='no',fase_solicitud='borrador' WHERE id_alumno=$idh";
                  $resids=$this->conexion->query($sql);
               }
            }
         ...
         foreach($hermanos_admision as $key=>$hermano)
         {
            ...
            if($existe_hermano==1)
            {
               ...
               else
               {
                  ...
                  $log->warning($ibaremo);
                  //$update=$this->conexion->query($ibaremo);
               }
               if($nh==0)
  ...
  public function save_tributantes($data,$log)
   ...
   public function genDatosHermanosAdmision($data,$log)
   {
      ...
      unset($data['id_alumno']);
		$log->warning("GENERANDO DATOS INSERCION HERMANOS ADMISION CONJUNTA:");
		$log->warning(print_r($data,true));
		$ncampos=9;
      $i=10;
		$guardar=1;
	   ...
	   if($id_usuario>0)
		{
         ...
         //grabamos datos del baremo y hermanos
			   ...
			   if($sol['conjunta']=='si')
            {
               ...
               $hermanos_admision=$this->getHermanos('admision',$id_alumno,$log_nueva);
				if($hbaremo=='si')
...
//comprobar si los datos corresponden a una solicitud existente
  public function compSol($d,$n,$a1) {
    ...
    public function getIdsHermanosAdmision($id,$log) 
	 {
      $resultSet=array();
      $log->warning("TIPO ADMISION");
      $squery="SELECT id_alumno FROM alumnos where id_alumno IN(SELECT id_hermano FROM alumnos_hermanos_admision ah WHERE ah.id_alumno=$id)";
      $log->warning("OBTENIENDO IDS HERMANOS ADMISION:  $squery");
      $query=$this->conexion->query($squery);
      $i=1;
      $sufijo=$tipo;
      ...
      $resultSet=array();
		...
		else if($tipo=='admision')
		{
         ...
         while ($row = $query->fetch_object()) 
         {
           ...
           $resultSet['hermanos_'.$sufijo.'_dni_alumno'.$i]=$row->dni_alumno;
		...
		else
		{
         ...
         $log->warning("HAFINALLLL");
         $log->warning(print_r($resultSet,true));
         $log->warning("FIN HAFINALLLL");
      return $resultSet;
	}
	...
	public function genOrdenSolicitudesAdmitidas($c=0,$tipo,$log) 
	{
      ...
      $delete="DELETE FROM alumnos_orden_$tipo WHERE id_centro=$c"; 
	...
	public function setSolicitudesSorteo($c=1,$solicitudes=0,$nvebo=0,$nvtva=0,$tipo,$log) 
	{
		$tabla='alumnos';
		$resultSet=array();
      ...
      $log->warning(print_r($ids,true));
		...
		else
		{
         //actualizamos campo de estado de la solicid despues del sorteo para marcar las sol admitidas, siempre excluyendo las borrador
         if(strlen($idsebo)>0)
            $sql_actestebo="update $tabla set est_desp_sorteo='admitida',id_centro_final=id_centro_destino where tipoestudios='ebo' and  fase_solicitud!='borrador' and id_alumno in(".$idsebo.")";
         if(strlen($idstva)>0)
            $sql_actesttva="update $tabla set est_desp_sorteo='admitida'i,id_centro_final=id_centro_destino where tipoestudios='tva' and fase_solicitud!='borrador' and id_alumno in(".$idstva.")";
		}
   ...
   public function getMaximoAleatorio()
	{
      $sql="SELECT MAX(nasignado) maxasignado FROM alumnos where fase_solicitud!='borrador' ";
		$query=$this->conexion->query($sql);
		if($query) {$row = $query->fetch_object();return $row->maxasignado;}
		else return 0;
	}
   ...
   //si estamos en peridodo de listados baremados provisionales accedemos a la tabla correspondiente
      $ares=array();
      $filtrorec='';
      if($estado_convocatoria<=ESTADO_ALEATORIO)
         $orden=" ORDER BY a.id_centro_destino, a.tipoestudios, a.apellido1,a.nombre,a.transporte asc,b.puntos_validados desc,b.hermanos_centro desc,b.proximidad_domicilio,b.renta_inferior,b.discapacidad_alumno,b.discapacidad_hermanos,b.tipo_familia_numerosa,b.tipo_familia_monoparental,a.nordensorteo asc,a.nasignado desc ";
      else
         $orden=" ORDER BY a.id_centro_destino,a.tipoestudios,a.transporte asc,b.puntos_validados desc,b.hermanos_centro desc,b.proximidad_domicilio,b.renta_inferior,b.discapacidad_alumno,b.discapacidad_hermanos,b.tipo_familia_numerosa,b.tipo_familia_monoparental,a.nordensorteo asc,a.nasignado desc ";
...
desc,a.tutores_centro desc,a.nordensorteo asc,a.nasignado desc";
		...
		if($tipo==0) //todas las solicitudes, incluyendo las q están en borrador
		{
			if($rol=='admin')
			{
				if($subtipo_listado=='dup') //solicitudes duplicadas
					$sql="select a.est_desp_sorteo,a.apellido1,a.apellido2,a.tipoestudios,a.fnac,a.dni_tutor1,a.nombre,a.id_alumno,a.reserva,c.nombre_centro FROM alumnos a join (select apellido1,nombre FROM alumnos group by apellido1,nombre having count(*)>1) dup on a.apellido1=dup.apellido1 and dup.nombre=a.nombre join centros c on c.id_centro=a.id_centro_destino join baremo b on b.id_alumno=a.id_alumno order by c.id_centro,a.tipoestudios, b.puntos_validados desc";
				else  //solicitudes normales
					$sql="SELECT c3.nombre_centro AS nombre_centro_final, a.est_desp_sorteo, a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.loc_dfamiliar,a.nordensorteo,a.nasignado as nasignado,a.reserva,b.*,c.nombre_centro,c.provincia,c2.nombre_centro as nombre_centro_origen, conjunta, c3.nombre_centro as nombre_centro_final FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro left join centros c2 on c2.id_centro=a.id_centro_estudios_origen LEFT JOIN centros c3 ON c3.id_centro=a.id_centro_final  order by c.id_centro desc,a.tipoestudios asc, b.puntos_validados desc";
			}
			else if($rol=='sp')
				$sql="SELECT c3.nombre_centro as nombre_centro_final, a.est_desp_sorteo,a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.loc_dfamiliar,a.nordensorteo,a.nasignado as nasignado,a.reserva,b.*,c.nombre_centro,c.provincia,c2.nombre_centro as nombre_centro_origen,conjunta FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro left join centros c2 on c2.id_centro=a.id_centro_estudios_origen left join centros c3 on c3.id_centro=a.id_centro_final where c.provincia='$provincia' order by c.id_centro,a.tipoestudios asc, b.puntos_validados desc";
			else
            $sql="SELECT a.est_desp_sorteo,a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.reserva,b.*,c.nombre_centro,c.provincia,c2.nombre_centro as nombre_centro_origen,conjunta FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro left join centros c2 on c2.id_centro=a.id_centro_estudios_origen  where c.id_centro=$c order by c.id_centro,a.tipoestudios asc, b.puntos_validados desc";
	...
	public function getSolicitudesFase2FinalesAdjudicadas($subtipo_listado,$rol,$id_centro,$estado_convocatoria,$log,$id_alumno) 
   {
      ...
      if($rol=='centro')
         $where=" AND id_centro_final=$id_centro AND est_desp_sorteo='admitida' AND tipo_inscripcion!='ordinaria' AND id_centro_estudios_origen!=id_centro_final AND c.nombre_centro!='NOCENTRO' ";
      if($rol=='admin')
      ...
      $resultSet=array();
		if($subtipo_listado=='lfinal_sol_ebo')
		   $sql="SELECT a.email as correo,a.tel_dfamiliar1 as telefono,'centrosdisponibles' as centrosdisponibles,a.id_alumno,a.tipoestudios,a.id_centro_final, a.apellido1,a.apellido2,a.nombre,c.nombre_centro as centro_definitivo,cc.nombre_centro as centro_solicitado FROM alumnos a,centros c,centros cc WHERE c.id_centro=a.id_centro_final AND cc.id_centro=a.id_centro_destino AND a.tipoestudios='ebo' $where order by a.id_centro_final desc, a.tipoestudios asc,a.nordensorteo asc,a.transporte asc";
   	elseif($subtipo_listado=='lfinal_sol_tva')
		   $sql="SELECT a.email as correo,a.tel_dfamiliar1 as telefono,'centrosdisponibles' as centrosdisponibles,a.id_alumno,a.tipoestudios,a.id_centro_final, a.apellido1,a.apellido2,a.nombre,c.nombre_centro as centro_definitivo,cc.nombre_centro as centro_solicitado FROM alumnos a,centros c,centros cc WHERE c.id_centro=a.id_centro_final AND cc.id_centro=a.id_centro_destino AND a.tipoestudios='tva' $where order by a.id_centro_final desc, a.tipoestudios asc,a.nordensorteo asc,a.transporte asc";
	...
	public function getSolicitudesFase2FinalesNoMatricula($subtipo_listado,$rol,$id_centro,$estado_convocatoria,$log,$id_alumno) 
   {
      //alumnos que no requieren matrícula por qudarse en el centro original
		$log->warning("\nCONSULTA SOLICITUDES FASE2 FINAL ESTADO $estado_convocatoria rol $rol\n");
      $where="";
	...
	public function getSolicitudesFase2FinalesDesplazados($subtipo_listado,$rol,$id_centro,$estado_convocatoria,$log,$id_alumno) 
   {
		$log->warning("\nCONSULTA SOLICITUDES FASE2 FINAL ESTADO $estado_convocatoria rol $rol\n");
      $where="";
      //mostramos los alumnos q eleigieron ese centro o q lo han obtenido en la fase final
      if($rol=='centro')
         $where=" AND id_centro_destino=$id_centro AND est_desp_sorteo='admitida' AND tipo_inscripcion!='ordinaria' ";
      if($rol=='admin')
         $where=" AND est_desp_sorteo='admitida' AND tipo_inscripcion!='ordinaria'";
	...
	public function getSolicitudesFase2($subtipo_listado,$rol,$id_centro,$estado_convocatoria,$log) 
   {
      ...
      $orden=" ORDER BY nombre_centro,tipoestudios,ao.transporte asc,ao.puntos_validados desc,ao.hermanos_tutores desc,ao.conjunta desc,ao.proximidad desc,ao.renta desc,ao.sobrevenida desc,ao.discapacidad desc,ao.familia desc,ao.orden asc";
		
      if($subtipo_listado=='lfase2_sol_ebo')
		   $sql="SELECT a.calle_dfamiliar,a.localidad,'centrosdisponibles' as centrosdisponibles, a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.centro_origen,a.id_centro_origen,a.nombre_centro,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.puntos_validados,b.puntos_validados as puntos_validados_originales,a.id_centro,a.centro1,a.centro2,a.centro3,a.centro4,a.centro5,a.centro6,a.centro_definitivo,a.id_centro_definitivo, tipo_modificacion,reserva FROM alumnos_fase2 a left join baremo b on b.id_alumno=a.id_alumno LEFT JOIN alumnos_orden_definitivos ao ON ao.id_alumno=a.id_alumno where a.tipoestudios='ebo' $orden";
   	elseif($subtipo_listado=='lfase2_sol_tva')
  			$sql="SELECT a.calle_dfamiliar,a.localidad, 'centrosdisponibles' as centrosdisponibles, a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.centro_origen,a.id_centro_origen,a.nombre_centro,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.puntos_validados,b.puntos_validados as puntos_validados_originales,a.id_centro,a.centro1,a.centro2,a.centro3,a.centro4,a.centro5,a.centro6,a.centro_definitivo,a.id_centro_definitivo,tipo_modificacion,reserva FROM alumnos_fase2 a left join baremo b on b.id_alumno=a.id_alumno LEFT JOIN alumnos_orden_definitivos ao ON ao.id_alumno=a.id_alumno where a.tipoestudios='tva' $orden";
		elseif($subtipo_listado=='csv_fase2') //para el csv 
      {
		   $sql="SELECT a.calle_dfamiliar,a.localidad, 'centrosdisponibles' as centrosdisponibles, a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.centro_origen,a.id_centro_origen, a.nombre_centro,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,a.puntos_validados,b.puntos_validados as puntos_validados_originales,a.id_centro,a.centro1,a.centro2,a.centro3,a.centro4,a.centro5,a.centro6,a.centro_definitivo,a.id_centro_definitivo, a.tipo_modificacion,a.reserva,a.reserva_original  FROM alumnos_fase2 a left join baremo b on b.id_alumno=a.id_alumno  LEFT JOIN alumnos_orden_definitivos ao ON ao.id_alumno=a.id_alumno $orden";
      }
...
as nasignado,c.nombre_centro, a.puntos_validados,a.id_centro_destino as id_centro  FROM $tabla_alumnos a left join centros c on a.id_centro_destino=c.id_centro where fase_solicitud!='borrador' and( estado_solicitud='duplicada' or estado_solicitud='irregular')";
			else
			   $sql="SELECT a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado, a.puntos_validados FROM $tabla_alumnos a left join centros c on a.id_centro_destino=c.id_centro where fase_solicitud!='borrador'  and( estado_solicitud='duplicada' or estado_solicitud='irregular') and c.id_centro=$c";
      }
	...
	public function getSolicitudesProvisionales($c=1,$tipo=1,$subtipo_listado,$estado_convocatoria=0,$rol,$log) 
	{
      ...
      {
		   if($rol=='admin' or $rol=='sp')
			   $sql="SELECT a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,c.nombre_centro,a.puntos_validados,a.id_centro_destino as id_centro FROM $tabla_alumnos a LEFT JOIN alumnos_orden_provisional ao ON ao.id_alumno=a.id_alumno, centros c WHERE a.id_centro_destino=c.id_centro AND fase_solicitud!='borrador' and estado_solicitud='apta' and est_desp_sorteo='admitida' $orden";
			else
			   $sql="SELECT a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,c.nombre_centro,a.puntos_validados,a.id_centro_destino as id_centro FROM $tabla_alumnos a LEFT JOIN alumnos_orden_provisional ao ON ao.id_alumno=a.id_alumno, centros c WHERE a.id_centro_destino=c.id_centro AND fase_solicitud!='borrador' and estado_solicitud='apta' and est_desp_sorteo='admitida' and c.id_centro=$c $orden";
      }
		elseif($subtipo_listado=='noadmitidos_prov')
      {
		   if($rol=='admin' or $rol=='sp')
			   ...
			   $sql="SELECT a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,c.nombre_centro,a.puntos_validados,a.id_centro_destino as id_centro FROM $tabla_alumnos a LEFT JOIN alumnos_orden_provisional ao ON a.id_alumno=ao.id_alumno, centros c WHERE a.id_centro_destino=c.id_centro AND fase_solicitud!='borrador' and estado_solicitud='apta' and est_desp_sorteo='noadmitida' and c.id_centro=$c $orden";
      }
		elseif($subtipo_listado=='excluidos_prov')
      {
          if($rol=='admin' or $rol=='sp')
            $sql="SELECT a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado
as nasignado,c.nombre_centro, a.puntos_validados,a.id_centro_destino as id_centro  FROM $tabla_alumnos a LEFT JOIN alumnos_orden_provisional ao ON ao.id_alumno=a.id_alumno, centros c WHERE a.id_centro_destino=c.id_centro AND fase_solicitud!='borrador' AND( estado_solicitud='duplicada' or estado_solicitud='irregular') $orden";
         else
            $sql="SELECT a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado, a.puntos_validados FROM $tabla_alumnos a LEFT JOIN alumnos_orden_provisional ao ON ao.id_alumno=a.id_alumno,centros c WHERE a.id_centro_destino=c.id_centro AND fase_solicitud!='borrador' and( estado_solicitud='duplicada' or estado_solicitud='irregular') and c.id_centro=$c $orden";
	...
	public function getMatriculadosAdjudicadosServProvincial($subtipo_listado,$rol,$log) 
	{
      $resultSet=array();
		if($rol=='centro')
	...
	public function getSolicitudesMatriculaFinal($c=1,$subtipo_listado,$estado_convocatoria,$log,$rol,$id_alumno,$provincia) 
	{
      ...
      if($rol=='admin')
      ...
      else if($rol=='centro')                                                                                                                                                                                                                         $sql="SELECT a.tel_dfamiliar1,a.matricula,a.email,a.id_alumno,a.nombre,a.apellido1,a.apellido2,a.tipoestudios,a.fase_solicitud,a.estado_solicitud,a.transporte,a.nordensorteo,a.nasignado as nasignado,c.nombre_centro,b.puntos_validados FROM alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_final=c.id_centro WHERE fase_solicitud!='borrador' AND estado_solicitud='apta' AND c.id_centro=$c  order by tipoestudios";
      else if($rol=='alumno')
         ...
         $sql="SELECT a.*,b.*,c.* FROM centros c,alumnos a,baremo b WHERE c.id_centro=a.id_centro_final AND a.id_alumno=b.id_alumno AND fase_solicitud!='borrador' AND estado_solicitud='apta' AND provincia='$provincia' AND id_centro_estudios_origen!=id_centro_final $order"; 
         //$sql="SELECT * FROM alumnos af JOIN centros c ON af.id_centro_final=c.id_centro AND fase_solicitud!='borrador' AND estado_solicitud='apta' and provincia='$provincia' AND id_centro_estudios_origen!=id_centro_final ORDER BY id_centro_final,af.tipoestudios ";
...
as nasignado,c.nombre_centro, a.puntos_validados,a.id_centro_destino as id_centro  FROM $tabla_alumnos a left join baremo b on b.id_alumno=a.id_alumno left join centros c on a.id_centro_destino=c.id_centro where fase_solicitud!='borrador' and( estado_solicitud='duplicada' or estado_solicitud='irregular') $order";
	...
	public function actualizaBaremo($t,$d,$log) 
	{
      $idh=$this->getIdFromToken($t);
      $sql="";
		$sol_completa=array();
		$query="SELECT b.* FROM alumnos a WHERE a.id_alumno=$id";
		$log->warning("CONSULTA SOLICITUD BAREMO: ".$query);
		$soldata=$this->conexion->query($query);
      if($data->num_rows>0)
      {
     	   while($row = $soldata->fetch_object()) 
	...
	public function getDatosBaremo($id,$log) 
	{
		$sol_completa=array();
		$query="SELECT b.* FROM alumnos a WHERE a.id_alumno=$id";
		$log->warning("CONSULTA SOLICITUD BAREMO: ".$query);
   ...
   public function generarSolicitudHermano($sol,$dh,$id_usuario,$tipo='save',$log)
   {
      ...
      if($tipo=='update')
      {
         ...
         if($sol['id_centro_destino']!='')
            $sol['id_centro_destino']=$this->getCentroId($dh['id_centro_destino'],$log);
      }
      else
      {      
         $sol['nombre']=$dh[0];
   ...
   {
      $query="select id_centro from centros where nombre_centro='".trim($nombre,'*')."'";
