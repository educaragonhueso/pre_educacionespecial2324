...
require_once $_SESSION['dir_base']."/config/config_global.php";
...
$err=$nombre_usuario_err = $clave1_err =$clave2_err= "";
  ...
  if(strlen(trim($_POST["clave1"])!=trim($_POST["clave2"])))
       $err = 'Las contraseñas son distintas';
        // Si no hay errores, actualizar la abse de dato
   if(empty($clave1_err) && empty($clave2_err) && empty($err) )
	{
		//si es un alumno la clave es de 4 digitos
      $id_centro=$_SESSION['id_centro'];
      $usuario=$_SESSION['id_centro'];
      $clave1=$_POST['clave1'];
      $sql1 = "UPDATE centros set primera_conexion='no' where id_centro=?";
      $sql2 = "UPDATE usuarios set clave=md5('".$clave1."'),clave_original='$clave1' where id_usuario=?";
  
      $log_acceso->warning("ENTRANDO CENTRO PRIMERA CONEXION: ".$sql1."\n".$sql2);
      $log_acceso->warning("CENTRO -- USUARIO : ".$id_centro." -- ".$usuario);

	   //if($stmt1 = $conexion->prepare($sql1) && $stmt2 = $conexion->prepare($sql2))
	   if($stmt1 = $conexion->prepare($sql1))
		{
         // Bind variables to the prepared statement as parameters
        $stmt1->bind_param("i", $id_centro);
        if($stmt1->execute())
        {
            $res1=1;
            $stmt1->close();
